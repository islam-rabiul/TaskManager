  ...
  template: `
    <div class="fade-in">
      <div class="section-header">
        ...
        <p class="section-sub">Tasks assigned to you.</p>
      </div>
      ...
      <div class="filter-bar">
        ...
        <div class="search-wrap">
          ...
          <input class="search-input" type="text" placeholder="Search tasks…" [(ngModel)]="searchQuery" id="my-tasks-search" />
        </div>
      </div>

      @if (loading()) {
        <div style="display:flex;align-items:center;gap:10px;padding:32px 0;color:var(--text-secondary)">
          <span class="spinner"></span> Loading your tasks…
        </div>
        ...
        <div class="task-grid">
          @for (t of filtered(); track t.taskId) {
            <app-task-card [task]="t" (cardClick)="openModal($event)" />
          }
        </div>
      }
    </div>
...
export class MyTasksComponent implements OnInit {
  ...
  filtered = computed(() => {
    let tasks = this.taskSvc.myTasks();
  ...
  reload(): void {
    ...
    this.taskSvc.fetchMyTasks().subscribe({
